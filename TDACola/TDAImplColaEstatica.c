...
booleano desencolar(Cola* pc, void* elem, size_t tamElem)
{
    ...
    desencolarSegmentado(pc, tamElem, tamElemEncolado, sizeof(size_t));
...
void encolarSegmentado(Cola* pc, const void* dato, size_t tamDato)
{
    ...
    tamDisponibleContiguo = pc->tamDisponible > pc->fondo ? pc->tamDisponible : TAM_COLA - pc->fondo;
    ...
    if(cantACopiar2 > 0)
    {
        memcpy(pc->vec + pc->fondo, dato + cantACopiar1, cantACopiar2); //no entiendo
